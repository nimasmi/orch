        ...
        def dec(func):
            ...
            def tag_func(context):
                if settings.ESI_ENABLED:
                    ...
                    # This allows the ESI middleware to know when to set the
                    # X-ESI header on the response
...
def purge_esi():
    from orch.utils.cache import purge_cache_on_all_sites

    for name in ESI_REGISTRY:
        # TODO: might need a separate domain for ESI and call
        # wagtail.contrib.wagtailfrontendcache.utils.purge_url_from_cache
